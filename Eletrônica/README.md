...
O cérebro da empilhadeira é a sua parte eletrônica. Esta seção documenta a alimentação, os
reguladores de tensão, os motores/drivers, os microcontroladores e os sensores utilizados no
...
- [2.2 Reguladores de tensão](#22-reguladores-de-tensão)
...
estimados dos motores sob carga (consumo estimado do sistema entre 8–15 A dependendo do modo
de operação).
...
Como a alimentação de 12,6 V das baterias é inconsistente para os demais componentes, são
utilizados reguladores de tensão para estabilizar a alimentação e proteger componentes sensíveis:

| Regulador | Função | Tensão de saída |
|---|---|---|
| **MP2307** | Alimenta o Raspberry Pi | 12 V → 5V |
| **LM2596** | Alimenta demais periféricos (ex: câmera) | 12V → 5,3 V |

> Componentes escolhidos por serem opções populares, simples e bem documentadas.
...
- **MPU-6050** — acelerômetro + giroscópio + sensor de temperatura. Os dados são lidos pelo
ESP32 via I²C e filtrados no Raspberry Pi com um **filtro de Kalman** para reduzir ruído e obter
estimativas estáveis de orientação (roll/pitch) e velocidade angular.
...
- *(Futuro, se necessário)* **AS5600** — encoder magnético para o motor do garfo (JGY-370), que
não possui encoder integrado de fábrica.
...
outro regulado para os microcontroladores/sensores.
...
Baterias (12,6V) ──┬──► Motores (tração + garfo)
                    └──► Reguladores 5V/3,3V ──► RPi / ESP32 / Sensores
...
| Regulador | MP2307 | Buck converter · 12V → 5V (Raspberry Pi) |
| Regulador | LM2596 | 12V → 5,3V (periféricos/câmera) |
...
| Sensor | MPU-6050 | IMU (acelerômetro + giroscópio + temperatura) |
| Sensor | Câmera | Detecção de AprilTags |
| Sensor (futuro) | AS5600 | Encoder magnético para o garfo |

---

*Documento gerado a partir do Relatório Pré-Projeto — ENG4061, PUC-Rio.*
