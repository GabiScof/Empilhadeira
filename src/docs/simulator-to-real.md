# Simulador → Robô Real — O Que Fizemos e O Que Aproveitamos

> Documento de referência para a equipe entender **exatamente** o papel do simulador,
> o que foi validado nele, e o que transfere (ou não) para `SIM=0` no hardware físico.
...
(câmera + ESP32) plugado nos **mesmos buracos** que o robô real usa.
...
**No robô real:** o mundo físico é a arena de verdade. O mapa JSON continua existindo,
...
**No robô real:** o ESP32 roda o firmware C++ de verdade. O emulador **serve para
desenvolver a lógica do Pi** e calibrar ganhos iniciais — mas a sintonia PID final
**tem que ser feita no hardware** (Ziegler-Nichols no chão).
...
### 2.5 FaultInjector — laboratório de falhas

**Exclusivo da simulação.** Permite testar modos de segurança sem destruir hardware:
...
| `vision_blur/drop` | Robustez do EKF e navegação |
...
cabos soltos). Não há API para injetá-las — você as provoca fisicamente.

---

## 3. O que roda **idêntico** nos dois modos (APROVEITAMOS 100%)
...
| Módulo | Arquivo | Função |
|--------|---------|--------|
| **Control Loop** | `tasks/control_loop.py` | Orquestra tudo @20 Hz — **crítico** |
...
## 4. O que **NÃO** aproveitamos no robô real
...
| Mapas JSON atuais (`corredor_pequeno`, etc.) | Coordenadas da simulação — **recriar medindo a arena** |
| Intrínsecos placeholder da câmera | `camera_intrinsics.json` está com `null` |
| Ruído/FOV/alcance simulados | Valores chute — medir câmera real |
| Ganhos PID/NAV/EKF provisórios | Ponto de partida — **recalibrar no chão** |

---

## 5. O que **transferimos com ajuste** (parâmetros compartilhados)
...
| Telemetria instável em MANUAL | Só sim (double-noise) — **não afeta real** |
...
| Navegação (25 testes) | ✅ Lógica — ⚠️ ganhos podem precisar ajuste |
| EKF (predição, correção, outlier) | ✅ Lógica — ⚠️ ruído real diferente |
| Missão em 4 mapas simulados | ✅ Fluxo SM — ⚠️ mapa real precisa existir |
| Integração sim (converge ZREF) | ✅ Comportamento — ⚠️ dinâmica real difere |
...
A única diferença visível para a lógica: **de onde vêm** `Sensors` (enc, mpu) e
`VisionState` / `TagObservation`.

---

## 8. Matriz de decisão — "Posso confiar nisso no real?"
...
Coisas que **só o hardware** pode validar:
...
- [ ] `camera_intrinsics.json` — calibração xadrez
...
| Joystick manual | ✅ |
| Garfo + fim-de-curso | ✅ |
| AUTOMATICO com 1 clique | ✅ |
| Convergência Z ≈ 15 cm | ✅ medir com fita |
| Ocultar tag → PARADO latched | ✅ |
| Desconectar serial → motores param | ✅ |
| Missão pick-place completa | ✅ com mapa real |
...
**Construímos um simulador que substitui câmera e ESP32, validou toda a lógica do Pi
(controle, navegação, missão, EKF, segurança, frontend) com 162 testes e cenários
de convergência — e vamos para o robô real reutilizando 100% dessa lógica, trocando
só os encaixes de hardware e recalibrando parâmetros físicos que o sim não pode medir.**
