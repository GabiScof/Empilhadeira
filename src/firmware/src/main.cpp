/**
 * main.cpp — Loop principal do ESP32.
 *
 * Duas cadencias: PID ~100 Hz (encoders + motores + garfo) e serial ~20 Hz
 * (MPU-6050 + telemetria ao Pi). Setpoints chegam via UART sem bloqueio;
 * watchdog para em SETPOINT_TIMEOUT_MS se o Pi parar de enviar.
 */
#include <Arduino.h>
#include <Wire.h>
...
// +-2g (16384 LSB/g), +-250 deg/s (131 LSB/(deg/s)), temp: raw/340 + 36.53
...
// AX,AY,AZ (6) + TEMP (2) + GX,GY,GZ (6) = 14 bytes
...
void setup() {
  ...
  // Acorda o MPU-6050 (+-2g, +-250 deg/s).
  mpuWake();
...
void loop() {
  ...
  // Watchdog: sem setpoint novo dentro do timeout -> estado seguro.
