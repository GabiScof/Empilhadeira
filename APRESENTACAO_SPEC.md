# SPEC DE APRESENTAÇÃO — Empilhadeira Robótica Autônoma

> **Instruções para quem for gerar a apresentação:** este documento é a fonte
> única e completa do conteúdo. A apresentação deve ser dividida em **3 partes:
> (1) Visão, (2) Controle, (3) Comunicação/Conexão**, precedidas de uma
> abertura com a visão geral do sistema e encerradas com status/lições
> aprendidas. O projeto **não está 100% implementado no hardware** — isso deve
> ser apresentado com honestidade: a lógica está completa e validada em
> simulação e bancada; a autonomia no chão ainda não rodou. O tom é de
> engenharia: decisões justificadas, problemas reais encontrados e como foram
> resolvidos. Todos os números abaixo são reais, extraídos do código e dos
> documentos do repositório.

---

## 0. VISÃO GERAL DO PROJETO (slides de abertura)

**O que é:** empilhadeira robótica em escala reduzida que transporta pallets
...
**Filosofia de desenvolvimento — simulação primeiro (sim-to-real):**
...
serial → 15–16/06 simulação completa + frontend v1 → 23/06 grande integração
(EKF, planejador, missão, mapas, docs) → 03/07 calibração real da câmera +
correções → **06/07 dia de bancada no robô real** (encoders, motores, sinais,
v_máx medida, dock-to-tag, D-pad).

**Estado em 2026-07-06 (para o slide de status):** eletrônica validada na
...
de testes ainda não rodaram no hardware** — a autonomia completa só existe
verde em simulação e testes de unidade.
...
## PARTE 1 — VISÃO (e tudo relacionado)
...
2. **Global:** "onde EU estou no mapa?" → cada tag detectada tem pose conhecida
...
```
Câmera USB (640×480) → grayscale → pupil-apriltags (tag25h9, PnP com
...
- **Detector:** `quad_decimate=1.0`, `refine_edges=1`, `decode_sharpening=0.25`,
...
### 1.3 Calibração da câmera (feita de verdade, 03/07)

- Método: tabuleiro de xadrez OpenCV (`findChessboardCorners` +
  `calibrateCamera`), **28 fotos a 640×480**, cantos internos 8×5, quadrado de
  3 cm.
- Resultado: **erro de reprojeção 0,144 px** (excelente).
- Intrínsecos: `fx=799,39 · fy=794,28 · cx=399,04 · cy=273,19`, com
  coeficientes de distorção completos `[k1,k2,p1,p2,k3]`.
- Existe uma **calibração alternativa por fotogrametria (3DF Zephyr)**
  registrada como plano B no próprio JSON (`fx=fy=833,1`) — se o teste da fita
  métrica a 30 cm bater melhor com ela, é só reverter.
- **Regra dura:** a resolução de captura TEM que ser a mesma da calibração
  (640×480 no `.env`); com `REQUIRE_CAMERA_CALIBRATION=1` o backend **se
  recusa a subir** em modo real sem calibração válida — segurança por design.
...
- **Correção** quando vê tag: pose absoluta por PnP contra o mapa; ruído de
  ...
  **gating de Mahalanobis a 3σ** rejeita detecções absurdas (falso positivo
  não teleporta o robô).
...
(`tag_hidden`, `vision_blur`, `vision_drop`). Foi assim que os 9/9 cenários de
aproximação foram validados antes de existir robô.

### 1.7 Dock-to-tag (novidade de 06/07 — ponte entre visão e missão)

Modo opt-in (`DOCK_TO_TAG=1`): o robô vê UMA tag, planeja **uma única vez**
uma rota de segmentos discretos (FORWARD/TURN) até um standoff de 15 cm de
frente para a tag e executa **por odometria/EKF** — exatamente a mesma
maquinaria da missão (SegmentExecutor), sem pick/place. Por isso é o ensaio
perfeito: se o dock funciona, a navegação da missão funciona. Robusto por
construção a perder a tag numa curva de 90° (não é servo contínuo). Dois
...
### 1.8 Problemas reais de visão e como foram resolvidos (slide forte)

1. **Sinal do X invertido (o bug que faria o robô fugir da tag)** — o frame
   óptico do OpenCV tem x positivo = tag à DIREITA; a convenção do projeto
   (herdada do simulador e da navegação) é x positivo = tag à ESQUERDA. Sem a
   negação na fronteira (`pose.py`), a autonomia real viraria **para longe**
   da tag — em simulação nunca apareceu porque a visão sintética já nascia na
   convenção certa. Corrigido em 06/07; lição: converta convenções **na
   fronteira** entre mundos, e valide sinais no hardware um a um.
...
4. **OpenCV quebrado no Pi** — instalações duplicadas/parciais do OpenCV
   ...
   cedo com diagnóstico claro e receita de correção, e **auto-detecta modo
   headless** quando rodando via SSH (sem display).
5. **Resolução ≠ calibração** — o default do config era 1280×720 (resolução do
   script legado), mas a calibração é 640×480; `.env` operacional força
   640×480 com comentário explícito. Intrínsecos só valem na resolução em que
   foram medidos.
...
Regra de ouro: **o Pi nunca duplica o PID de roda** — a malha externa cuida de
...
- **PID por roda @100 Hz:** ganhos iniciais **Kp=20, Ki=5, Kd=1** (motores
  ...
  já visto a 8 rad/s).
- **PWM:** LEDC do ESP32 a **20 kHz** (acima do audível), 8 bits (0–255).
...
- **IMU MPU-6050 cru pela serial:** acelerômetro ±2g e gyro ±250°/s enviados
  crus (m/s², °/s) — **toda a fusão é no Pi** (decisão: manter o firmware
  burro e determinístico, filtros onde há float e teste fácil).
...
- **GyroCalibrator — calibração automática no boot (feature elegante):** com o
  robô parado ~2–3 s, usa a **gravidade** para descobrir sozinho qual eixo do
  ...
  **não importa** — pode remontar o sensor que nada quebra. Ritual de boot:
  robô imóvel em chão firme por 3 s (pneus macios balançam o chassi; calibrar
  balançando = heading derivando).
...
- **SegmentExecutor (malha externa):** FORWARD com `v = K_DIST·dist` e correção
  ...
  gira primeiro); TURN em de pé até tolerância de ~2°; tolerância de posição
  2 cm; timeout de segmento → FAULT da missão.
- **Navegação reativa legada (1 tag): APPROACH → FACE → RETREAT**, controle
  ...
  mode-switching reativo é o certo para robô diferencial com câmera frontal;
...
- **Máquina de missão pick-and-place:**
  ...
  robô para de verdade e espera o operador acionar o garfo e clicar
...
### 2.4 Números de calibração (medidos, não chutados)

| Grandeza | Valor | Como |
|---|---|---|
| v_máx real no chão | **24,0 cm/s** | cronometrado: 100 cm em 4,16 s a talo cheio (06/07) |
...
| Raio da roda | 2,7 cm | medição da equipe; confirmar por teste de ROLAGEM (pneu esmaga com peso) |
...
### 2.5 Problemas reais de controle e as histórias por trás (slides fortes)

1. **Canais dos motores TROCADOS na fiação (o melhor bug da bancada, 06/07):**
   ...
   Sintoma bizarro: uma roda saturava no máximo e a outra morria, alternando o
   ...
   software** no `config.h`. **Lição de metodologia: sempre testar um lado por
...
2. **Encoder esquerdo sobrecontando ~420 pulsos/volta:** estava nos GPIOs
   ...
   De quebra, a decodificação passou de x1 para **x4 completa** com tabela de
   transição, que rejeita bounce por construção.
3. **"Testing without the encoder" — parada incondicional em setpoint zero:**
   com um encoder morto (medida=0), o comando de parar tinha erro 0 mas a
   integral acumulada segurava o duty no máximo — **o robô ignorava o STOP**.
   Correção de segurança no firmware: setpoint 0 = parada incondicional que
   *bypassa* o PID e reseta a integral. O robô ficou seguro e dirigível mesmo
   com encoder com defeito.
4. **MPU-6050 dormindo / barramento I2C caindo:** duas assinaturas distintas
   diagnosticadas no stream: (a) tudo-zero com `temp = 36,53 °C` = sensor
   DORMIU (I2C responde 14 bytes zerados; 36,53 é o offset da fórmula de
   temperatura com raw=0 — o "tell" perfeito); (b) tudo-zero com `temp = 0` +
   erro do driver Wire = **barramento caiu** (contato/EMI perto do L298n).
   Correções: firmware **auto-recupera** re-enviando o wake após ~1 s de
   leituras mortas; e o GyroCalibrator no Pi **descarta frames mortos**
   (‖accel‖ < 2 m/s² é fisicamente impossível com sensor vivo — a gravidade
   nunca some) para não corromper a média de gravidade nem erodir o bias.
...
7. **Ganhos de bancada ≠ chão (assumido no prontuário):** atrito estático faz
   o robô não partir com talo baixo (esperado, não é bug — integral acumula
   devagar); bateria caindo faz o PID saturar antes ("robô amolece").

### 2.6 O que a simulação pegou antes do hardware (evidência do sim-first)
...
parada falsa a 24–25 cm (dead zone com histerese)… — nenhum deles precisou de
robô físico para ser encontrado.

---

## PARTE 3 — COMUNICAÇÃO / CONEXÃO

### 3.1 Os 4 contratos congelados (única fonte de verdade)
...
três — por isso o doc é "fonte única de verdade".
...
- **Robustez por descarte:** os decodificadores dos dois lados são
...
### 3.4 Cadeia de segurança fim-a-fim (slide-chave: defesa em profundidade)
...
- Validado na bancada: Ctrl-C com rodas girando → parada **< 200 ms** ✅.

### 3.5 D-pad "Comando exato" (06/07 — instrumento de teste, não brinquedo)

Painel com botões que enviam **vetores puros**: Frente = ω exatamente 0
(reta perfeita), Gira = v exatamente 0 (giro no lugar), em 30/60/100% da
velocidade máxima — tira a "mão trêmula" do joystick da equação nos testes de
retidão e odometria (1 m medido com fita; 360° e conferir o heading).
Detalhe de engenharia: cada botão re-envia o vetor a cada **100 ms**
(heartbeat) para satisfazer o command watchdog de 400 ms; soltou → envia
`{0,0}` na hora.

### 3.6 Duas topologias de rede (decisão operacional)

- **MODO DEV** (desenvolvendo): frontend servido pelo Mac (`npm run dev`,
...
- **MODO OPERAÇÃO** (demo/desafio): `npm run build` no Mac, `rsync` do `dist/`
  ...
  **sem Node/npm no Pi**, uma porta só, e o WebSocket resolve sozinho para o
  host da página. Celular abre `http://<IP_DO_PI>:8000/` e pronto.

### 3.7 Problemas reais de comunicação e como foram resolvidos
...
6. **Porta serial é uma só:** monitor do PlatformIO, script de bancada e
   backend brigam pela UART (`Device or resource busy`) — virou "regra de
   ouro" operacional no plano de testes: fecha um antes de abrir o outro.
...
## 4. STATUS HONESTO (slide final antes das lições)
...
- Câmera calibrada de verdade (0,144 px) com plano B registrado.
- v_máx medida no chão (24 cm/s) e configurada com folga (19 cm/s).
- Dock-to-tag + vista de cima real implementados e verdes em teste de unidade.
...
- Fase 2 no chão: manual completo (retidão, ré, sinais de giro no chão),
  garfo com carga, watchdogs andando de verdade, sanidade de odometria
  (1 m e 360°).
...
- **Fase 3 inteira (autonomia) nunca rodou no hardware**: aproximação reativa
...
raio efetivo desigual entre pneus, bateria caindo ("robô amolece"), Wi-Fi do
local com RTT > 400 ms, escorregamento nas curvas de 90° degradando odometria,
tag impressa fora de medida. Cada um com sintoma e ação já escritos.

---

## 5. LIÇÕES DE ENGENHARIA (fechamento sugerido)

1. **Simulação com as MESMAS interfaces paga o investimento:** dezenas de bugs
   de lógica morreram antes do robô existir; no hardware sobraram só os bugs
   de hardware (fiação, sinais, ruído) — exatamente os que a sim não pega.
2. **Convenções de sinal são a fronteira mais perigosa do sim-to-real:**
...
6. **Parâmetro medido > parâmetro chutado:** v_máx cronometrada com fita e
   gravada a 80%; nada de inventar valor (política explícita "NÃO INVENTAR
   VALOR" com `TODO(equipe)` em cada placeholder).
7. **Diagnóstico escrito vira ativo:** as duas assinaturas de falha do MPU e o
   prontuário de sintomas transformam o próximo bug de horas em minutos.
